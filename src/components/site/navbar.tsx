...
export function Navbar() {
  return (
    <header className="sticky top-0 z-50 border-b border-[rgba(255,255,255,0.08)] bg-[rgba(7,7,8,0.86)] backdrop-blur-md">
        ...
        <div className="hidden items-center gap-7 md:flex">
          {navigationItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="text-sm text-[var(--gralith-dark-text-secondary)] transition-colors hover:text-[var(--gralith-dark-text)]"
            >
              {item.label}
            </Link>
          ))}
